#!/bin/bash
# SessionEnd hook - saves session-end segment with sequential numbering (stateless!)
# This script is called when a Claude Code session ends.
# It saves the final state of the transcript and associated files.
# Uses sequential segment numbering (segment-0, segment-1, ...) instead of segment-final.
...
# Get KB path (exit silently if not configured - session is ending anyway)
...
# Write segment metadata (session-end type with sequential number)
...
# Notify user (may not be visible if session exits quickly)
cat << EOF
{
  "continue": true,
